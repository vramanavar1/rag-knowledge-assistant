...
if [[ -n "$OPENAI_NAME" ]]; then
  ...
  for spec in "$EMBEDDING_DEPLOYMENT:text-embedding-3-small:OpenAI:1" \
              "$UTILITY_DEPLOYMENT:gpt-4o-mini:OpenAI:2024-07-18"; do
    IFS=: read -r name model fmt version <<<"$spec"
